...
class UserInterfaceNote(ABC):
    @abstractmethod
    def find_author(self, notes):
        pass
    
    @abstractmethod
    def note_show_all(self, notes):
        pass
    
    @abstractmethod
    def tag_find_and_sort(self, notes):
        pass


class ConsoleInterfaceNote(UserInterfaceNote):
        ...
        while tries > 0:
            try:
                ...
                for key in self.notes:
                    if key == note_name:
                        return {self.notes[note_name].name: self.notes[note_name]}
                if not note_name in self.notes:
                    raise ValueError(
    ...
    def tag_find_and_sort(self, notes):
        ...
        similar_dict = {}
        for key in self.notes:
            for tag in self.notes[key].tags:
                if tag.value == tag_name:
                    match_dict[self.notes[key].name] = self.notes[key]
                if tag_name in tag.value:
                    similar_dict[self.notes[key].name] = self.notes[key]
        if match_dict:
            ...
            suggested_dict = {}
            for key in self.notes:
                if self.notes[key].note.value.__contains__(tag_name):   
                    suggested_dict[self.notes[key].name] = self.notes[key]
            if suggested_dict:
